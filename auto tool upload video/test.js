...
(async () => {
    ...
    const cookiesString = fs.readFileSync("./cookies2.json");
    const cookies = JSON.parse(cookiesString);
    await page.setCookie(...cookies);

    await page.goto(
        "https://studio.youtube.com/channel/UCgmcl72rZ-S7Jwo4PGzGsnw/videos/upload?filter=%5B%5D&sort=%7B%22columnType%22%3A%22date%22%2C%22sortOrder%22%3A%22DESCENDING%22%7D",
        {
    ...
    const run = async () => {
        for (let i = valFile; i < mp4File.length; i++) {
            // for (let i = valFile; i < 90; i++) {
    ...
    try {
        ...
        console.log("error");
        // await page.click("#close-button > paper-ripple");
        // await run();
        await page.waitForTimeout(2000);
    }
})();
